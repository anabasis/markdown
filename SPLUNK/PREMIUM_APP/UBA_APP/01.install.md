...
Log in to the virtual machine as the caspida user using SSH. Specify caspida123 as the existing default password. You will be prompted to provide the default password a second time, and then change the existing password. For example:
...
Find the two additional 1TB disks using the sudo fdisk -l command. An example disk is /dev/sdb.
Format and mount the additional 1TB disks associated with the OVA.
Add the additional 1TB disk for Splunk UBA metadata storage. For example, using /dev/sdb as the 1TB disk:
...
Add the additional 1TB disk for Spark. The disk should be mounted as /var/vcap2. Use the /opt/caspida/bin/Caspida add-disk <device> <mount> command. For example:
...
Open an m4.4xlarge AWS instance.
...
Select System > Health Monitor to view the Health Monitor dashboard.
Review the status of your Splunk UBA installation.
Confirm that the system health is Normal and all services are running.
If some services are not running, wait 15 minutes for the system to stabilize and the Health Monitor page to refresh. If after 15 minutes, there are still some services not running, try restarting all services.
/opt/caspida/bin/Caspida stop-all
/opt/caspida/bin/Caspida start-all
